...
library PoolLibrary {
    ...
    function addLiquidity(
        ...
        if (currency == address(0)) {
            (, , liquidity) = IUniswapV2Router02(router).addLiquidityETH{
                value: liquidityEth
...
contract PresalePool is OwnableUpgradeable, IPool, ReEntrancyGuard {
    ...
    function initialize(
        ...
        // );
        require(
            _saleInfo[6] < _saleInfo[7],
            "End time must be after start time"
        );
        require(
            _saleInfo[8] >= MINIMUM_LOCK_DAYS,
    ...
    function initializeVesting(
        ...
        require(
            _vestingInit[0] >= 0 && _vestingInit[0] < 10_000,
            "Invalid bips for TGE"
        );
        require(
            _vestingInit[2] >= 0 && _vestingInit[2] < 10_000,
    ...
    function contribute(
        uint256 _amount
    ) public payable inProgress onlyWhitelisted {
    ...
    function _withdrawableTokens() internal view returns (uint256) {
        uint256 volume = purchasedOf[msg.sender];
        uint256 totalClaim = claimedOf[msg.sender];
        ...
        uint256 tgeReleaseAmount = FullMath.mulDiv(volume, tgeBps, 10_000);
        uint256 cycleReleaseAmount = FullMath.mulDiv(volume, cycleBps, 10_000);
        uint256 currentTotal = 0;
        if (block.timestamp >= tgeDate) {
            currentTotal =
                (((block.timestamp - tgeDate) / cycle) * cycleReleaseAmount) +
    ...
    function withdrawContribution() external noReentrant {
        ...
        require(
            ...
            "You Don't Have Enough contribution"
        );
        uint256 fees = 0;
        if (poolState == PoolState.inUse) {
            fees = feesWithdraw;
        ...
        totalVolumePurchased = totalVolumePurchased.sub(
            purchasedOf[msg.sender]
        );
    ...
    function finalize() external onlyOperator noReentrant {
        ...
        uint256 currencyAmount = currency == address(0)
        ...
        uint256 totalTokenSpent = liquidityToken.add(tokenFee).add(
            totalVolumePurchased
        );
        remainingToken += IERC20(token).balanceOf(address(this)).sub(
            totalTokenSpent
        );

        // Pay platform fees
        if (ethFee > 0) {
            if (currency == address(0)) payable(adminWallet).sendValue(ethFee);
            else IERC20(currency).safeTransfer(adminWallet, ethFee);
        }
        if (tokenFee > 0) {
        ...
        if (remainingEth > 0) {
            ...
            else IERC20(currency).safeTransfer(governance, remainingEth);
        }
    ...
    function withdrawLeftovers() external onlyOperator noReentrant {
        require(
            ...
            "It is not time to withdraw leftovers"
        );
        require(
            totalRaised < softCap,
            "Soft cap reached, call finalize() instead"
        );
        IERC20(token).safeTransfer(
            governance,
            IERC20(token).balanceOf(address(this))
    ...
    function getContributionAmount(
        ...
        return
            PoolLibrary.getContributionAmount(
                contributed,
                minContribution,
                maxContribution,
                availableToBuy
    ...
    function getUpdatedState()
        public
        view
        returns (uint256, uint8, bool, uint256, string memory)
    {
        return (
            totalRaised,
            uint8(poolState),
